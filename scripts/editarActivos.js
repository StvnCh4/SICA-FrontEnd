...
async function editarActivoFunc(id) {
    const queryURLActivo = `http://localhost:8080/activos/${id}`;
    ...
    .then(function(products){
        ...
        for(let product of products){
            ...
            document.getElementById("activoDescrip").value = `${product.descripcion}`;
            activePiso.options[activePiso.selectedIndex].text = `${product.piso}`;
            activeStatus.options[activeStatus.selectedIndex].text = `${product.estado}`;
            activeLocation.options[activeLocation.selectedIndex].text = `${product.ubicacion}`;
        }
    });
}
...
async function actualizarActivoFunc(id) {
    ...
    actualizarActivo.addEventListener("click", async function actualizarActivoFuct(e){
        e.preventDefault();

        const PUTActivo = `http://localhost:8080/activos/${id}`;
